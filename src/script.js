...
// Fog
const fog = new THREE.Fog(0x262837, 1, 21)
scene.fog = fog
...
door.position.z = 4 / 2 + 0.001 // Half of depth (z) of walls
door.position.y = 1
house.add(door)
...
for(let i = 0; i < 50; i++){
    ...
    grave.rotation.z = (Math.random() - 0.5 ) * 0.9
...
const tick = () =>
{
    ...
    ghost1.position.x = Math.cos(ghost1Angle) * (4.5 + Math.sin(elapsedTime * 1.5))
    ghost1.position.z = Math.sin(ghost1Angle) * (4.5 + Math.sin(elapsedTime * 1.9))
    ...
    ghost3.position.x = Math.cos(ghost3Angle) * (7 + Math.sin(elapsedTime * 1.5))
    ghost3.position.z = Math.sin(ghost3Angle) * (7 + Math.sin(elapsedTime * 1.9))
